...
%% Third Section 
\section{Methods}
We utilized several methods to compute the hidden angle measurement. Throughout this paper we use degrees in plots and examples, but all the measurements and computations were done in radians. We also used the average absolute angle error as our primary evaluation metric. This took the average of the absolute value of the circular distance between the true state and predicted state at each time step across the whole dataset.
...
gridsearch over several different possible values using the average absolute angle error. A filter was constructed for each hyperparameter
...
\subsection{Accuracy}
We focused on the accuracy of the following situations.
